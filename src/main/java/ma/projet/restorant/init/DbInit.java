package ma.projet.restorant.init;
...
public class DbInit implements CommandLineRunner { // Remove this at production
	...
	public void run(String... args) throws Exception {
		...
		this.userDetailsRepo.save(new UserDetails("NA", 'N', "NA", "NA",
				this.userRepo.save(new User("admin", passwordEncoder.encode("admin"), 1234, 0, "ADMIN", true))));
		this.userDetailsRepo.save(new UserDetails("NA", 'N', "NA", "NA",
				this.userRepo.save(new User("bidhan", passwordEncoder.encode("bidhan"), 1234, 1, "USER", true))));
		this.userDetailsRepo.save(new UserDetails("NA", 'N', "NA", "NA",
				this.userRepo.save(new User("john", passwordEncoder.encode("john"), 1234, 2, "USER", true))));
		this.userDetailsRepo.save(new UserDetails("NA", 'N', "NA", "NA",
				this.userRepo.save(new User("sam", passwordEncoder.encode("sam"), 1234, 3, "USER", true))));
